...
export class TaskService {
  ...
  getTask(id: string): Observable<Task> {
    ...
    .pipe(map(
      (response: Task) => {
        response = this.convertDatesWithTimeZoneBeforeToShow(response);
        return response;
    }));
  }
  ...
  updateTask(task: Task): Observable<Task> {
    task = this.changeDatesToGMTBeforeToSave(task);
  ...
  private convertDatesWithTimeZoneBeforeToShow(task: Task): Task {
    if (task.due) { task.due = TaskanaDate.transformDateWithTimeZone(task.due); }
    if (task.modified) { task.modified = TaskanaDate.transformDateWithTimeZone(task.modified); }
    if (task.completed) { task.completed = TaskanaDate.transformDateWithTimeZone(task.completed); }
    if (task.planned) { task.planned = TaskanaDate.transformDateWithTimeZone(task.planned); }
    if (task.claimed) { task.claimed = TaskanaDate.transformDateWithTimeZone(task.claimed); }
    if (task.created) { task.created = TaskanaDate.transformDateWithTimeZone(task.created); }
    return task;
  }

  private changeDatesToGMTBeforeToSave(task: Task): Task {
